...
│   │   ├── Settings/        SettingsShell, SettingsWindowController, LoginItemController
...
│   ├── Features/            Overview (popover; Styles/Glass, Styles/Terminal), Usage, Conversations, Status, Notifications, Settings
...
├── Packages/AgentBarKit/    everything that is not UI: models, readers, status pages, usage alerts, snapshot codec
...
AgentBar's panes are General, Appearance, Menu Bar, Agents, Status, Support, About. Menu Bar
and Status were carved out of Appearance and Agents once each had grown a second subject:
how the panel is painted is not the same question as what the menu bar item shows, and which
blocks the panel draws is not the same question as whether the services behind them are up.
...
- **Codex's reset credits** come from `chatgpt.com/backend-api/wham/rate-limit-reset-credits`,
  asked with the same sign-in and at the same moment as the usage, concurrently so the figures
  do not wait for it. The answer (11 Sep 2026) is `{"credits": [...], "available_count": n,
  "total_earned_count": n, ...}`, each credit with `status`, `granted_at` and `expires_at` (ISO
  8601, microseconds allowed). Only `status: available` is kept, and the panel counts those whose
  expiry is still ahead, since the account may call a credit available a moment after it
  expired. It is best effort: a failure costs nothing but the list, which stays as it was until
  the next answer, and a shape without `credits` reads as not answered rather than as none. Like
  the plan it is an inventory, not a reading, so a file written later does not push it out. It
  is drawn as the last line of the agent's group (`ResetCreditsRow`, `TerminalResetsRow`): the
  count, and on a click each credit's expiry and the time left. AgentBar never redeems one.
...
Settings › Agents reorders it by drag: each row carries a handle, `draggable` puts the
section's name on the pasteboard, and dropping onto another row takes that row's place
(`AppSettings.move(_:onto:)`). The rule is tested in `AgentBarTests/AppSettingsTests.swift`;
the gesture cannot be driven from a test, so what is tested is what the gesture asks for.

Stored as **names, not positions**: a block added by an update is in no stored list, and
joins the end in the canonical order rather than vanishing or landing somewhere arbitrary.
A name that no longer belongs to a block is dropped without leaving a hole.

A style is handed `OverviewContext.sections`; an empty list means "every agent, then the
conversations", which is what a preview or the screenshot harness wants without saying so.

## Service status

Quota answers "how much is left"; it cannot answer the question that comes first when an
agent misbehaves, which is whether the service is working at all. `StatusReader` reads each
agent's own status page, and `Notifier` says one short thing when it stops working and
one when it starts again. The second is the one the feature exists for: a status page tells
you a service is down, and then nothing tells you it is back, so you go and try until it is.

Nothing here is signed in to. These pages are public, the requests carry the app's name and
nothing else, and a test asserts that no credential can find its way into one.

### Where each agent is read from

All three publish a Statuspage-v2-compatible summary, so there is one parser and three
addresses (verified live, 4 Sep 2026):

| Agent | Page | Family | Components read |
|---|---|---|---|
| Claude | `status.claude.com/api/v2/summary.json` | Atlassian Statuspage | `Claude Code`, `Claude API` |
| Codex | `status.openai.com/api/v2/summary.json` | incident.io | `Codex API` |
| Cursor | `status.cursor.com/api/v2/summary.json` | Atlassian Statuspage | `CLI`, `IDE` |

**The components are the point.** A page states an overall indicator, but claude.ai going
down while Claude Code keeps working is not this agent's outage, and waking someone for it
...
### Traps in these feeds (seen 4 Sep 2026)

- `status.anthropic.com` **301s to `status.claude.com`**. The new host is written out.
- **OpenAI is incident.io, not Atlassian.** Its `summary.json` has no `incidents` and no
  `scheduled_maintenances` key at all, and `api/v2/incidents/unresolved.json` answers an
  HTML 404. A strict `Decodable` throws on it, so nothing is required.
- The page's own indicator and a component's status are **different vocabularies**:
  `none | minor | major | critical | maintenance` against
  `operational | degraded_performance | partial_outage | major_outage | under_maintenance`.
  `StatusPage` maps them separately onto `StatusLevel`.
- Atlassian marks a component group with `"group": true` and repeats its children below it;
  incident.io writes no `group` key. Groups are dropped either way.
- **`ETag` and `304` work on Atlassian only** (Claude and Cursor answered 304 to a
  conditional request; OpenAI sends no tag). The etag is kept per agent and sent as
  `If-None-Match`; the request sets `reloadIgnoringLocalCacheData` so `URLSession`'s own
  cache does not answer first and swallow the 304.
- Atlassian's CDN holds ten seconds (`s-maxage=10`) and its public status API is not rate
  limited, so a minute costs nothing and anything under ten seconds buys nothing.
...
- **Only an issue latches.** Maintenance and unknown change nothing in either direction.
- **A failed read is not an outage.** Dropped Wi-Fi looks exactly like a dead service; the
  last good status stays on screen with the problem said beside it.
- **Two consecutive readings must agree.** At the one-minute outage cadence that is two
  minutes of lateness in exchange for never flapping.

`StatusModel` asks every five minutes, and every minute while any watched agent is unwell,
so the moment a service returns is a minute away rather than five. It keeps asking with
nothing on screen: a recovery noticed only when the panel is next opened is not one anyone
was told about. Two switches in Settings › Agents say whether to announce a service going
...
`Notifier.isAvailable` checks that as well as `isRunningTests`. Permission is asked
when a switch is turned on, or when there is a first message to show; never at launch.

### Where it is drawn

`ServiceStatus` does not go into `Snapshot`, which is what one read of the agents' *files*
produced; it travels on `OverviewContext` instead, so the widget is untouched.
...
say what you already assumed. It earns its colour by changing.

At the other end of the same line is a `⋯` menu of the pages about that agent
(`Agent.links`): its own front door, the usage and status pages the panel's figures come
from, and its documentation. It replaced a link on the agent's name, which could only ever
...
A window's use is announced as it fills, at marks the user picks, for the windows the user
picks (Settings › Alerts). None is chosen on a fresh install: which windows are worth a
message is theirs to say. The marks start at 80% and 90%.

`UsageWatch` (AgentBarKit) decides, and like `StatusWatch` it holds no clock and posts
nothing: a snapshot's limits in, the alerts they earn out. Per window (`UsageLimit.id`) it
remembers the highest mark already announced in the window running now.

- **Once per mark and window.** 81% says 80% and every later read up to 90% says nothing. A
  read that jumps past two marks says the higher one, once.
- **A new window starts afresh.** The window's reset date moving on by more than half an
  hour is a new window (a real one moves by its whole length); so is a remembered reset date
  that has passed. A limit with no reset date, such as extra usage, starts afresh when its use
  falls five points below the mark, since use only falls when a window starts over.
- **History raises nothing.** A window that has rolled over describes a window that no
  longer exists; it is skipped and watched from the start again.
- **Turning a window off forgets it**, so turning it on again says where it stands. Adding a
  mark above where a window stands is announced when reached; one below says nothing.

`UsageAlerts` keeps the watch in UserDefaults (`usageAlertMarks`), so a restart part way
through a week has nothing new to say. `AppDependencies.observeUsageAlerts` feeds it every
snapshot and every change to the choices, and never the sample snapshot
(`AgentsModel.isShowingSample`, set with the snapshot because the setting changes before the
read does). `Notifier` posts one message per window under one identifier, so 90% replaces 80%
rather than stacking under it: "Codex · Weekly: 91% used", then "Past your 90% mark. Starts
over in 1d 4h, Thu 14:05." Permission is asked the first time a window is turned on.
