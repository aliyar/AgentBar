...
│   ├── Features/            Overview (popover; Styles/Glass, Styles/Terminal), Usage, Conversations, Status, Settings
...
├── Packages/AgentBarKit/    everything that is not UI: models, readers, status pages, snapshot codec
...
agent's own status page, and `StatusNotifier` says one short thing when it stops working and
