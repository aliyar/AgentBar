#!/usr/bin/env swift
// Generates the AppIcon PNG set: a rounded gradient square with the gauge glyph.
// Usage: swift scripts/make-icon.swift [output-appiconset-dir]
import AppKit

let output = CommandLine.arguments.count > 1
    ? URL(fileURLWithPath: CommandLine.arguments[1], isDirectory: true)
    : URL(fileURLWithPath: "AgentBar/Resources/Assets.xcassets/AppIcon.appiconset", isDirectory: true)
try? FileManager.default.createDirectory(at: output, withIntermediateDirectories: true)

let glyph = "gauge.with.dots.needle.33percent"
let top = NSColor(calibratedRed: 0.20, green: 0.72, blue: 0.62, alpha: 1)
let bottom = NSColor(calibratedRed: 0.05, green: 0.27, blue: 0.32, alpha: 1)
...
func drawMaster(size: CGFloat) -> NSImage {
    ...
    if let symbol = NSImage(systemSymbolName: glyph, accessibilityDescription: nil)?.withSymbolConfiguration(config) {
